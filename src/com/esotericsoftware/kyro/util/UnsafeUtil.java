...
import static com.esotericsoftware.kyro.util.UnsafeUtil.unsafe;
...
public class UnsafeUtil {
	...
	static {
		...
		try {
			if (!Util.isAndroid) {
				...
				if (TRACE) trace("kyro", "Running on Android platform. Use of sun.misc.Unsafe should be disabled");
			}
		} catch (java.lang.Exception e) {
			if (TRACE)
				trace("kyro", "sun.misc.Unsafe is not accessible or not available. Use of sun.misc.Unsafe should be disabled");
		}
	...
	public static Field[] sortFieldsByOffset (List<Field> allFields) {
		...
		for (Field f : allFields) {
			if (TRACE) trace("kyro", "Field '" + f.getName() + "' at offset " + unsafe().objectFieldOffset(f));
		}
