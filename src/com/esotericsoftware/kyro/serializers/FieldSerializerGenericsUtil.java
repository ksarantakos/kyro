...
final class FieldSerializerGenericsUtil {
	...
	Generics buildGenericsScope (Class clazz, Class[] generics) {
		...
		if (typeParams != null && typeParams.length > 0) {
			Generics genScope;
			trace("kyro", "Class " + clazz.getName() + " has generic type parameters");
			int typeVarNum = 0;
			...
			for (TypeVariable typeVar : typeParams) {
				...
				if (TRACE) {
					trace("kyro", "Type parameter variable: name=" + typeVarName + " type bounds=" + Arrays.toString(typeVar.getBounds()));
				}
				...
				if (concreteClass != null) {
					...
					if (TRACE) trace("kyro", "Concrete type used for " + typeVarName + " is: " + concreteClass.getName());
				}
	...
	private Class<?> getTypeVarConcreteClass (Class[] generics, int typeVarNum, String typeVarName) {
		if (generics != null && generics.length > typeVarNum) {
			...
			if (TRACE) trace("kyro", "Trying to use kyro.getGenericScope");
			Generics scope = kyro.getGenericsScope();
			if (scope != null) {
		...
		if (fieldGenericType != null) {
			if (fieldGenericType instanceof TypeVariable && serializer.getGenericsScope() != null) {
				...
				if (concreteClass != null) {
					...
					if (TRACE)
						trace("kyro", "Determined concrete class of '" + field.getName() + "' to be " + fieldClass[0].getName());
				}
				...
				if (actualTypeArguments != null) {
					...
					if (TRACE && fieldGenerics != null) {
						trace("kyro", "Determined concrete class of parametrized '" + field.getName() + "' to be " + fieldGenericType + " where type parameters are " + Arrays.toString(fieldGenerics));
					}
				}
				...
				// Kyro.getGenerics(fieldGenericType);
				if (TRACE && fieldGenerics != null) {
					trace("kyro", "Determined concrete class of a generic array '" + field.getName() + "' to be " + fieldGenericType + " where type parameters are " + Arrays.toString(fieldGenerics));
				} else if (TRACE) trace("kyro", "Determined concrete class of '" + field.getName() + "' to be " + fieldGenericType);
			}
		}
	...
	CachedField newCachedFieldOfGenericType (Field field, int accessIndex, Class[] fieldClass, Type fieldGenericType) {
		...
		if (TRACE) {
			trace("kyro", "Field '" + field.getName() + "' of type " + fieldClass[0] + " of generic type " + fieldGenericType);
		}

		if (TRACE && fieldGenericType != null)
			trace("kyro", "Field generic type is of class " + fieldGenericType.getClass().getName());
		...
		if (TRACE) {
			trace("kyro", "Generics scope of field '" + field.getName() + "' of class " + fieldGenericType + " is " + scope);
		}
		...
		if (fieldGenerics != null && cachedField instanceof ObjectField) {
			if (fieldGenerics.length > 0 && fieldGenerics[0] != null) {
				...
				if (TRACE) trace("kyro", "Field generics: " + Arrays.toString(fieldGenerics));
			}
		}
		return cachedField;
	}
	...
	public static Class[] getGenerics (Type genericType, Kyro kyro) {
		if (genericType instanceof GenericArrayType) {
			Type componentType = ((GenericArrayType)genericType).getGenericComponentType();
			if (componentType instanceof Class)
				return new Class[] {(Class)componentType};
			else
				return getGenerics(componentType, kyro);
		}
		if (!(genericType instanceof ParameterizedType)) return null;
		if (TRACE) trace("kyro", "Processing generic type " + genericType);
		Type[] actualTypes = ((ParameterizedType)genericType).getActualTypeArguments();
		Class[] generics = new Class[actualTypes.length];
		int count = 0;
		for (int i = 0, n = actualTypes.length; i < n; i++) {
			Type actualType = actualTypes[i];
			if (TRACE) trace("kyro", "Processing actual type " + actualType + " (" + actualType.getClass().getName() + ")");
			...
			if (actualType instanceof Class)
				...
				if (componentType instanceof Class)
					...
					Class[] componentGenerics = getGenerics(componentType, kyro);
