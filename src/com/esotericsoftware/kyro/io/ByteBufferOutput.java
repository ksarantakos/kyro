...
import com.esotericsoftware.kyro.util.UnsafeUtil;
...
public class ByteBufferOutput extends Output {
	...
	public int writeVarInt (int val, boolean optimizePositive) throws KyroException {
		niobuffer.position(position);
...
// value.getBytes(charIndex, charIndex + charsToWrite, buffer, position);
	 ...
	 * @param optimizePositive If true, small positive numbers will be more efficient (1 byte) and small negative numbers will be
	 *           inefficient (9 bytes). */
	public int writeLongS (long value, boolean optimizePositive) throws KyroException {
		if (!optimizePositive) value = (value << 1) ^ (value >> 63);
		if (value >>> 7 == 0) {
			require(1);
			niobuffer.put((byte)value);
