...
public class InputChunked extends Input {
	...
	private void readChunkSize () {
		try {
			...
			for (int offset = 0, result = 0; offset < 32; offset += 7) {
				...
				if (b == -1) throw new KyroException("Buffer underflow.");
				result |= (b & 0x7F) << offset;
				if ((b & 0x80) == 0) {
					chunkSize = result;
					if (TRACE) trace("kyro", "Read chunk: " + chunkSize);
					return;
				}
			}
		...
		throw new KyroException("Malformed integer.");
	}
	...
	public void nextChunks () {
		...
		while (chunkSize > 0)
			skip(chunkSize);
		chunkSize = -1;
		if (TRACE) trace("kyro", "Next chunks.");
	}
}
